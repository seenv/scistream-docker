...
class ProxyContainer():
    ...
    def update_listeners(self, listeners, s2ds_proc, uid, role = "PROD"):
        ...
        if role == "PROD":
            ...
            vm_name = "s2cs-p"      # the name of the s2cs container shouldn't change
        else:
...
class Haproxy(ProxyContainer):
    def __init__(self, role, service_plugin_type="docker"):
        ...
        if role == "PROD":
            self.container_name = "myhaproxy-prod"
        else:
            self.container_name = "myhaproxy-cons"
...
class JanusPlugin():
    ...
    def start(name, container_config):
        ...
        if role == "PROD":
            ...
            dest_path = Path("/data/scistream-demo/configs/haproxy-prod.cfg")
        else:
            cfg= Path(__file__).parent / "haproxy-cons.cfg"
            dest_path = Path("/data/scistream-demo/configs/haproxy-cons.cfg")
