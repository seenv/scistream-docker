...
class ProxyContainer():
    ...
    def start(self, num_conn, listener_ip):
        ...
        # finding total of num_conn free port
        start_port=5070
        end_port=65535
        free_ports = []
        
        for port in range(start_port, end_port):
            if len(free_ports) == num_conn:
                break
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                try:
                    ...
                    continue
        if len(free_ports) < num_conn:
            print(f"Warning: Only {len(free_ports)} free ports found out of {num_conn} requested.")
        print(f"Free ports found: {free_ports}")

        self.local_ports = free_ports
