...
class ProxyContainer():
    def __init__(self, service_plugin_type="docker"):
        ...
        self.instance_counter = 0  # Counter to increment for each instance

        pass

    def generate_unique_name(self, base_name):
        # Use timestamp or counter to make the name unique
        timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
        unique_name = f"{base_name}_{timestamp}_{self.instance_counter}"
    ...
    def update_listeners(self, listeners, s2ds_proc, uid, role = "PROD"):
        ...
        # Set a unique container name based on the base name
        self.container_name = self.generate_unique_name("myproxy")
