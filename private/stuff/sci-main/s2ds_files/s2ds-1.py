...
class Haproxy(ProxyContainer):
    ...
    def __init__(self, service_plugin_type="docker"):
        self.service_plugin_type = service_plugin_type
        self.image_name = 'haproxy:latest'
        self.container_name = f"myhaproxy_{role.lower()}"
        self.cfg_filename = f"haproxy_{role.lower()}.cfg"
        self.key_filename = f"haproxy_{role.lower()}.cfg.j2"
        self.cfg_location = f"/usr/local/etc/haproxy/haproxy_{role.lower()}.cfg"
        self.key_location = f"/usr/local/etc/haproxy/haproxy_{role.lower()}.key"
        if self.service_plugin_type == "dockersock":
            self.cfg_filename = f"/data/scistream-demo/configs/haproxy_{role.lower()}.cfg"
        pass


class Nginx(ProxyContainer, role):
    """
    def __init__(self, service_plugin_type="docker"):
        self.service_plugin_type = service_plugin_type
    ...
    def __init__(self, service_plugin_type="docker", role):
        self.service_plugin_type = service_plugin_type 
        self.image_name = 'nginx:latest'               
        self.container_name = f"mynginx_{role.lower()}"
        self.cfg_filename = f"nginx_{role.lower()}.conf"
        self.key_filename = f"nginx_{role.lower()}.conf.j2"
        self.cfg_location = f"/etc/nginx/nginx_{role.lower()}.conf"
        self.key_location = f"/etc/nginx/nginx_{role.lower()}.key"
        if self.service_plugin_type == "dockersock":
            self.cfg_filename = f"/data/scistream-demo/configs/nginx_{role.lower()}.conf"
        pass
...
class Stunnel(ProxyContainer):
    ...
    def __init__(self, service_plugin_type="docker", role):
        self.service_plugin_type = service_plugin_type
        self.image_name = 'stunnel:latest'
        self.container_name = f"mystunnel_{role.lower()}"
        self.cfg_filename = f"stunnel_{role.lower()}.conf"
        self.key_filename = f"stunnel_{role.lower()}.key"
        self.cfg_location = f"/etc/stunnel/stunnel_{role.lower()}.conf"
        self.key_location = f"/etc/stunnel/stunnel_{role.lower()}.key"
        if self.service_plugin_type == "dockersock":
            self.cfg_filename = f"/data/scistream-demo/configs/stunnel_{role.lower()}.conf"
            self.key_filename = f"/data/scistream-demo/configs/stunnel_{role.lower()}.key"
        pass 
